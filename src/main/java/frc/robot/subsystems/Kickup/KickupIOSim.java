...
/**
 * Simulation implementation of KickupIO.
 * 
 * This class simulates the kickup wheel physics for testing and development
...
public class KickupIOSim implements KickupIO {
    ...
    public KickupIOSim() {
        // Create wheel simulation using X60 Kraken motor

        LinearSystem<N1, N1, N1> kickupFlywheelPlant =
            LinearSystemId.createFlywheelSystem(
                DCMotor.getKrakenX60(1),
                KickupConstants.GEAR_RATIO,
                KickupConstants.SIM_MOI
            );

        wheelSim = new FlywheelSim(kickupFlywheelPlant, DCMotor.getKrakenX60(1), ShooterConstants.TOP_FLYWHEEL_GEAR_RATIO);
    ...
    public void updateInputs(KickupIOInputs inputs) {
        ...
        if (closedLoopMode) {
            ...
            double maxAccel = KickupConstants.ACCELERATION * 0.02; // acceleration * dt
            ...
            double ff = currentSetpoint * KickupConstants.kV 
                + Math.signum(currentSetpoint) * KickupConstants.kS;
        ...
        // Update simulation
        wheelSim.setInputVoltage(appliedVolts);
        ...
        inputs.statorCurrentAmps = wheelSim.getCurrentDrawAmps();
    ...
    /**
     * Gets the simulated wheel velocity in rotations per second.
     * Converts from rad/s to rps.
     */
    private double getVelocityRPS() {
        return wheelSim.getAngularVelocityRPM() / 60.0;
    }
}